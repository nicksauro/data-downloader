"""data_downloader.cli.contracts — sub-app ``contracts`` (Story 1.6).
...
Microcopy IDs (R17): ``BTN_LIST_CONTRACTS``, ``BTN_VALIDATE_CONTRACT``,
``ERR_INVALID_CONTRACT``, ``HLP_CONTRACTS``, ``HLP_VALIDATE``,
``EMP_CONTRACTS_LIST``.
"""
...
from data_downloader.cli._helpers import _get_credential, _open_catalog

__all__ = ["contracts_app", "register"]
...
def register(app: typer.Typer) -> None:
    """Registra o sub-app ``contracts`` no ``app`` raiz."""
...
def contracts_validate(
    ...
    # Story v1.0.2 B2: leitura via _get_credential com fallback p/ naming legado.
    ...
    try:
        with ProfitDLL() as dll:
            ...
            # Q-DRIFT-02: timeout 300s — handshake MARKET_DATA pode levar >60s.
