...
- **Fix-followup (task #21b, 2026-05-12 — Dex):** o singleton reusa a instância **no MODO em que foi inicializada** (ignora `**init_kwargs` se a instância já existe). Como "Testar Conexão" inicializava com `minimal_handshake=True` e "Baixar" usa `minimal_handshake=False` (modo completo), o cenário "Testar Conexão → Baixar" deixava o download rodando sobre uma DLL em modo minimal-probe → ela crashava INTERNAMENTE ao traduzir trades (`ConnectorMarketDataLibraryU.TranslateTrade | Access violation ... PopulateTradeV0`, "SILENT MODE" — não mata o processo) → `status=failed trades=0` (repro `docs/qa/repro-q08e.log`). Fix: `data_downloader.dll.session.resolve_dll_init_mode()` — fonte única do modo de init (lê `DATA_DOWNLOADER_DLL_MINIMAL_HANDSHAKE`), usada por AMBOS os call sites → modos sempre concordam. `get_dll()` agora também loga `warning dll.session.mode_mismatch` se uma chamada subsequente pedir um modo divergente (reusa no modo original, não re-inicializa).
