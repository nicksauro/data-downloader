...
| Throughput escrita Parquet sustentado (production writer) | >= 100k trades/s | **measured-vectorized ✅** | 2.2 measured (v2.0.0-vectorized) | Achieved p50 = **121_565 trades/s** (+21.6% acima do target; speedup 4.40x vs v1.1.0-mock 27_638). Story 2.2 fechou gap via vectorização interna (pa.compute + DuckDB ROW_NUMBER). Schema/fronteira preservados. JSON: `benchmarks/results/baselines_v2_vectorized/bench_parquet_write-2.0.0-vectorized.json`. |
