#!/usr/bin/env python3
"""Probe MINIMAL ctypes puro — variante PETR4 (Bovespa) — Story 1.7d.

Sanity check: discriminar H1 (conta sem permissao de historico) vs H4
(contrato WDOJ26 vencido). Se PETR4 retorna trades > 0, conta TEM permissao
de historico — bug e por contrato vencido (H4).

Espelha probe_history_minimal.py exatamente, trocando apenas:
  - ticker: WDOJ26 -> PETR4
  - exchange: F (BMF/futuros) -> B (Bovespa/equities)

Ver profitdll/Exemplo Python/main.py:651 — "MGLU3" usa exchange "B".
"""
...
def main() -> int:
    ...
    print("[INFO] PROBE PETR4 (Bovespa) — sanity check H1 vs H4", flush=True)
    ...
    # ----- DIFF vs probe_history_minimal.py: PETR4 / B (Bovespa) -----
    ticker = "PETR4"
    exchange = "B"
    ...
    if trades_received > 0:
        print(
            f"[VERDICT] PETR4_OK => trades={trades_received}. "
            f"Conta TEM permissao de historico — H1 REFUTADA.",
            flush=True,
        )
        return 0
    else:
        print(
            "[VERDICT] PETR4_ZERO => zero trades em PETR4 tambem. "
            "H1 (sem permissao) ou problema mais amplo.",
            flush=True,
        )
        return 10
